...
Sect.~\ref{sec:alignment-method}, and a wider similarity search with the same
number of grid points (nine scales and seven rotations), spanning
\(s\in[0.6,1.4]\) and \(\phi\in[-12^\circ,12^\circ]\), fixed in the
frozen experiment contract before any result was computed. The wider grid
therefore also has coarser spacing (0.10 in scale and \(4^\circ\) in rotation,
versus 0.05 and \(1^\circ\) in the production grid); this comparison does not
isolate search range from discretization.
...
clustered by target character \cite{efron1993bootstrap}. The two frozen
scores also have nominal, unclustered two-sided Spearman \(p\) values for
\(\rho=0\); interpretation emphasizes the character-cluster intervals.
Inter-rater
