...
\begin{figure*}[!t]
  \centering
  \includegraphics[width=\textwidth]{figures/revision/figure1_pipeline.pdf}
  \caption{Reference-conditioned structural assessment.
  (a)~Three of the seven natural same-character cross-style pairs of the
  external reference library (Fig.~\ref{fig:data-resources}c), labelled with
  their target character; the left column holds the regular-script Ouyang Xun
  reference and the right column the running-script Wang Xizhi candidate of
  each pair. Each row is recognizable yet structurally different, and the
  first row is followed through panels (b)--(e). (b)~The candidate
  ...
  available before text generation. The cross-style pairs are illustrative;
