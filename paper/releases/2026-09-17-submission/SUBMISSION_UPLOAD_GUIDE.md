...
1. **图片许可**。投稿信里目前写的是中性表述——说明了 CC BY-NC 4.0 许可、
   我们署名、未再分发数据集、如编辑部需要会提供许可证明。**没有**声明"已
   获得许可"（我不会替你们写不属实的话）。`cover_letter.tex` 里我用注释留了
   三种替换版本：
   - 已拿到书面回复 → 追加"已获得许可，往来邮件作为 related file 上传"；
   - 仍在等回复 → 追加"已发函申请许可，收到后转发"；
   - 被拒绝 → 删掉 Fig. 1 / Fig. 3(c) / Fig. 5 里相关图并重跑绘图脚本。

   拿到回复后告诉我，我把对应版本启用、重编译、刷新哈希。

2. **三张证件照 + 简介确认**。IJDAR 作者须知明确要求 50–100 词简介和护照
   尺寸黑白照。简介已写好（`paper/AUTHOR_BIOGRAPHIES.md`），需要三位本人
   确认措辞并提供照片。注意冯缘那份我已经改成女性代词。

3. **跟院里确认伦理声明措辞**。现在写的是 "Not applicable" 且未引用校内
   规定。若华师大要求写具体的豁免依据，告诉我改哪里。

4. **原创性声明确认**。投稿信里写了"未发表、未他投、没有需要申报的同作者
