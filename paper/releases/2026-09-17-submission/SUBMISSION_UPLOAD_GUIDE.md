...
1. **主路径：投稿界面里填（这条才算数）。** 在 SNAPP 的 `Authors` 标签页
   **点 Edit 展开作者表单**，逐位作者的编辑框里有 ORCID 字段，填写 16 位的
   iD（形如 `0000-0002-1825-0097`，末位可能是 `X`）。Springer 会把这里登记的
   iD 与作者账号绑定，**最终发表的文章上显示的 ORCID 来自这里**，不来自
   我们的 LaTeX 源码。通讯作者通常还要求先用 ORCID 登录一次。
   → 若找得到就三位都填。**但这不是必填项**：期刊 Instructions for Authors
   写的是 *"If available, the 16-digit ORCID of the author(s)"*。若整个流程
   里都找不到该字段，投稿不受影响，Springer 会在接受后的生产阶段收集链接
   （2026-09-19 更正：此前写成"必须做"，表述过强）。
