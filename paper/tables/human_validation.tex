\begin{table}[t]
\caption{Association between the structural scores and the mean rating of
three blinded calligraphy-domain raters across 150 canonical pairs.
Confidence intervals
use 10,000 bootstrap resamples clustered by character; \(p\) values are
nominal, unclustered two-sided Spearman tests.}
\label{tab:human-validation}
\centering
\scriptsize
\setlength{\tabcolsep}{3pt}
\begin{tabular}{lccc}
\toprule
Score & \(\rho\) & 95\% CI & \(p\) \\
\midrule
Production & 0.297 & [0.148, 0.441] & \(2.28\times10^{-4}\) \\
Coverage-aware & 0.429 & [0.313, 0.535] & \(4.32\times10^{-8}\) \\
\bottomrule
\end{tabular}
\end{table}
